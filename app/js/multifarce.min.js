 ...
 *               ServiceClient
 */
jQuery.easing.easeInQuart=function(e,f,a,h,g){return h*(f/=g)*f*f*f+a};jQuery.fn.formattedText=function(b){var a="<p>"+this.text(b).html()+"</p>";a=a.replace(/\*([^*\r\n]+)\*/g,"<strong>$1</strong>");a=a.replace(/_([^_\r\n]+)_/g,"<em>$1</em>");a=a.replace(/(\r\n|[\r\n]){2,}/g,"</p><p>");a=a.replace(/(\r\n|[\r\n])/g,"<br/>");return this.html(a)};jQuery.fn.hasEvent=function(b){var a=this.data("events");return a&&a[b]};var Multifarce=(function(a,f,k,g,i){var c="multifarce",e=function(){var n=arguments[0],m;for(m=1;m<arguments.length;m++){arguments[m].remove()}this.current=function(o){if(o){return o==n}return n};this.show=function(o){if(n==o){return}n.replaceWith(o);n=o}},d=(function(){var m=function(){i.call(this,"/api/")};m.prototype={createCommand:function(o,n,t,q,s,r,p){this.simpleCall("create_command",{frame:o,commands:n,text:t,go_to_frame:q,flags_on:s,flags_off:r,flags_required:p})},createFrame:function(o,n){this.simpleCall("create_frame",{title:o,text:n})},execute:function(p,o,n){this.simpleCall("execute",{frame:p,command:o,flags:n})},getCommand:function(n){this.simpleCall("get_command",{command:n})},getFirstFrame:function(){this.simpleCall("get_first_frame",{})},getFrame:function(n){this.simpleCall("get_frame",{frame:n})},getFrames:function(){this.simpleCall("get_frames",{})},getStatus:function(n){this.simpleCall("get_status",{path:n})},getUserInfo:function(n){this.simpleCall("get_user_info",{user:n})},logIn:function(o,n){this.simpleCall("log_in",{username:o,password:n})},logOut:function(){this.simpleCall("log_out",{})},register:function(q,n,p,o){this.simpleCall("register",{username:q,display_name:n,password:p,email:o})},onSuccess:null,onSuccessBind:null,onError:null,onErrorBind:null,error:function(n,o){this.onError=n;this.onErrorBind=o;return this},success:function(o,n){this.onSuccess=o;this.onSuccessBind=n;return this},simpleCall:function(o,n){this.call(o,n,this.onSuccess,this.onSuccessBind,this.onError,this.onErrorBind);this.onSuccess=null;this.onSuccessBind=null;this.onError=null;this.onErrorBind=null}};return m})(),h=new d(),j=(function(){var o={},m,n=function(t){f.call(this,"load");var q=false,r,p=function(){this.clearHandlers();if(t){delete o[t]}},s=function(u){r=u;q=true;this.raise("load")};this.get_displayName=function(){return r.display_name};this.get_emailHash=function(){return r.email_md5};this.get_username=function(){return r.username};this.isCurrent=function(){return this==m};this.isLoaded=function(){return q};if(!t){this.get_email=function(){return r.email};this.get_googleEmail=function(){return r.google_email};this.get_googleLogUrl=function(){return r.google_login||r.google_logout};this.get_googleNickname=function(){return r.google_nickname};this.get_type=function(){return r.type};this.googleLoggedIn=function(){return r.google_logged_in};this.loggedIn=function(){return r.logged_in}}this.load=function(u){if(u){s.call(this,u);return}h.success(s,this).error(p,this);if(t){h.getUserInfo(t)}else{h.getStatus()}}};n.current=function(){if(!m){m=new n()}return m};n.get=function(p){if(typeof p!="string"){throw"Invalid type (username); expected string."}if(p in o){return o[p]}else{o[p]=new n(p)}};n.logIn=function(r,p){var q=n.current();if(q.loggedIn()){alert("You're already logged in!");return}h.success(function(s){q.load(s)});h.logIn(r,p)};n.logOut=function(){var p=n.current();if(!p.loggedIn()){alert("You're not logged in!");return}if(p.googleLoggedIn()){location.href=p.get_googleLogUrl()}h.success(function(){p.load()});h.logOut()};return n})(),b=(function(){var m=function(r){f.call(this,"execute-success","execute-error","frame-load");var t,q,o,s=function(u){this.raise("execute-error",u)},p=function(u){if(u.frame_id!=t){t=u.frame_id;r.success(n,this).getFrame(t)}o=u.flags;this.raise("execute-success",u)},n=function(u){t=u.id;q=u.title;this.raise("frame-load",u)};this.get_frameId=function(){return t};this.get_frameTitle=function(){return q};this.get_state=function(){return t+","+o.join(",")};this.set_state=function(w){var v=w.split(","),x=parseInt(v.shift(),10);if(x&&x!=t){for(var u=v.length-1;u>0;u--){if(!v[u]){v.splice(u,1)}}l.start(x,v)}};this.execute=function(u){if(!u){return}if(!t){this.raise("execute-error",{message:"The game has not been started!"});return}r.error(s,this).success(p,this);r.execute(t,u,o)};this.start=function(u,v){t=u||0;o=v||[];r.success(n,this);if(t>0){r.getFrame(t)}else{r.getFirstFrame()}}};return m})(),l=new b(h);return{init:function(){var D=g("div.page"),Z=g("#notifications"),V=g("#page-name"),S=g("#username"),y=g("#avatar img"),af=g("#home"),m=af.find("#current-frame"),q=m.find("h3"),Q=m.find("div.text"),ab=af.find("#action"),t=af.find("#action-go"),ac=af.find("#log"),E=g("#log-in"),u=g("#log-in-go"),U=g("#log-in-username"),R=g("#log-in-password"),H=g("#log-out"),aj=g("#new-command"),v=g("#new-command-frame"),O=g("#new-command-1"),N=g("#new-command-2"),L=g("#new-command-3"),K=g("#new-command-4"),I=g("#new-command-5"),F=g("#new-command-text"),ai=g("#new-command-go-to-frame"),r=g("#new-command-create"),P=g("#new-frame"),ag=g("#new-frame-title"),p=g("#new-frame-text"),s=g("#new-frame-create"),ah=g("#not-found"),W=g("#not-found-path"),aa=g("#register"),J=g("#register-go"),B=g("#register-username"),ae=g("#register-name"),G=g("#register-email"),z=g("#register-password"),o=new e(af,E,H,aj,P,ah,aa),Y=j.current(),M=function(al,ak){if(al){V.text(al);document.title=al+" - "+c}if(ak){o.show(ak)}},X=function(){var ak=ab.val().toLowerCase();if(!ak){return}ab.val("");ac.prepend(g('<div class="command"/>').text("> "+ak));if(ak=="reset"||ak=="start"){ac.prepend(g('<div class="result"/>').text("Starting..."));l.start()}else{l.execute(ak)}ab.focus()},x=function(al,ak){Z.empty();Z.append(g("<p/>").addClass(ak||"message").append(g('<a href="#"/>').click(function(){g(this).closest("p").remove();return false}).text(al)).hide().fadeIn(500).animate({opacity:0},15000,"easeInQuart",function(){g(this).remove()}))},C=a.handler(function(ak){}),A=a.handler(function(){var ak=l.get_frameId(),al=this.get_param("state");if(al){l.set_state(al)}else{if(ak>0){g.hash.go("?state="+l.get_state())}}M(l.get_frameTitle()||"Hello World!",af);if(!ab.hasEvent("keydown")){ab.keydown(function(am){if(am.keyCode==13){X()}});t.click(X)}}),n=a.handler(function(){M("Log in",E);u.click(function(){j.logIn(U.val(),R.val());U.val("");R.val("")})}),ad=a.handler(function(){M("Log out",H);j.logOut()}),T=a.handler(function(){v.find("option").remove();ai.find("option").remove();h.success(function(al){for(var ak=0;ak<al.length;ak++){v.append(g("<option/>").val(al[ak].id).text(al[ak].title));ai.append(g("<option/>").val(al[ak].id).text(al[ak].title))}}).getFrames();M("Creating new command",aj);r.click(function(){var ak=[];if(O.val()){ak.push(O.val())}if(N.val()){ak.push(N.val())}if(L.val()){ak.push(L.val())}if(K.val()){ak.push(K.val())}if(I.val()){ak.push(I.val())}h.success(function(){alert("Success!")}).createCommand(parseInt(v.val()),ak,F.val(),parseInt(ai.val()))})}),w=a.handler(function(){M("Creating new frame",P);s.click(function(){h.success(function(){alert("Success!")}).createFrame(ag.val(),p.val())})});NotFoundHandler=a.handler(function(){W.text(this.get_path());M("Path not found!",ah)}),RegisterHandler=a.handler(function(){M("Register",aa);J.click(function(){h.success(function(){alert("Success!");if(Y.googleLoggedIn()){Y.load();g.hash.go("")}else{g.hash.go("log-in")}});h.register(B.val(),ae.val(),z.val()||null,G.val())})}),UserHandler=a.handler(function(ak){}),site=new a([["^$",A],["^commands/(\d+)$",C],["^commands/new$",T],["^frames/new$",w],["^log-in$",n],["^log-out$",ad],["^register$",RegisterHandler],["^user/([^/]+)$",UserHandler],["^.*$",NotFoundHandler]]);g("body").ajaxStart(function(){g(this).addClass("loading")}).ajaxStop(function(){g(this).removeClass("loading")});g(document).hashchange(function(al,ak){site.exec(ak)});g.hash.init();Y.listen("load",function(){if(this.loggedIn()){S.text(this.get_displayName());y.attr("src","http://www.gravatar.com/avatar/"+this.get_emailHash()+"?s=28&d=identicon&r=PG")}else{S.text("Not logged in");y.attr("src","http://www.gravatar.com/avatar/?s=28&d=identicon&r=PG")}var ak,al;if(this.googleLoggedIn()){ak="div.google";al="div.not-google"}else{ak="div.not-google";al="div.google"}if(this.loggedIn()){ak+=", div.multifarce";al+=", div.not-multifarce"}else{ak+=", div.not-multifarce";al+=", div.multifarce"}D.find(ak).each(function(){var am=g(this),an=am.data("hidden");if(an){am.append(an)}am.removeData("hidden")});D.find(al).each(function(){var am=g(this);if(am.data("hidden")){return}am.data("hidden",am.children().remove())});D.find("a.google-accounts").attr("href",this.get_googleLogUrl())});Y.load();l.listen("frame-load",function(ak){if(l.get_frameId()){g.hash.go("?state="+l.get_state())}M(ak.title,af);m.stop(true).animate({height:0,opacity:0},400).queue(function(){q.text(ak.title);Q.formattedText(ak.text);m.css({position:"absolute",height:"auto"});m.animate({height:m.height(),opacity:1},1000);m.css({height:0,position:"static"});m.dequeue()});ac.prepend(g('<div class="frame"/>').append(g('<p class="entering"/>').append("Entering ").append(g("<strong/>").text(ak.title))).append(g("<div/>").formattedText(ak.text)))});l.listen("execute-error",function(ak){x(ak.message,"error");ac.prepend(g('<div class="result"/>').text(ak.message))});l.listen("execute-success",function(ak){g.hash.go("?state="+l.get_state());x(ak.text,"success");ac.prepend(g('<div class="result"/>').text(ak.text))});ab.focus()},api:h,game:l,user:j}})(Application,EventSource,Hash,jQuery,ServiceClient);jQuery(Multifarce.init);